...
This document provides a detailed technical overview of the SafeHaven Insurance Admin catalog project. It is written to guide developers, through the architecture, design patterns, and code implementations used in this project.
...
We use an SQLite database (`safehaven.db`) or an in-memory database solution
...
A global controller advice [GlobalExceptionHandler.java] is annotated with `@RestControllerAdvice`.
...
- **Fix**: Make sure you don't have multiple database viewers or applications writing to `safehaven.db` or in-memory database simultaneously. Hibernate manages SQLite connections safely when running the Spring Boot app.
